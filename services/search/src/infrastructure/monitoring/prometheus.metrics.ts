...
export class PrometheusMetrics implements IMetricsService {
  ...
  recordEsOperation(operation: 'search' | 'bulk_upsert' | 'bulk_delete', status: 'success' | 'error', durationSeconds: number): void {
    this.esOps.labels({ operation, status }).observe(durationSeconds);
  }

  recordWorkerBatch(status: 'success' | 'error', durationSeconds: number, recordsProcessed: number): void {
