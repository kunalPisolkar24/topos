...
- **A `.env` file** with your app secrets (usually at `infrastructure/docker/prod/.env`)
...
dry-run     /topos/frontend/config (8 keys)
dry-run     /topos/frontend/config (2 keys)
dry-run     /topos/frontend/config (2 keys)
...
found 8 allowlisted keys in .env: GITHUB_ID, GITHUB_SECRET, GOOGLE_ID, ...
created    /topos/frontend/config (8 keys)
created    /topos/frontend/config (2 keys)
created    /topos/frontend/config (2 keys)
...
aws --endpoint-url http://localhost:4566 --region ap-south-1 \
  secretsmanager get-secret-value --secret-id /topos/frontend/config --query SecretString
```
...
sequenceDiagram
    ...
    You->>Tool: --env-file infrastructure/docker/prod/.env --endpoint-url http://localhost:4566
...
The `.env` file doesn't contain any keys the tool knows about. Make sure you're pointing to the right file (`infrastructure/docker/prod/.env`) and that it contains keys like `NEXTAUTH_SECRET`, `GOOGLE_ID`, etc.
...
The `.env` file doesn't exist at the specified path. Check `--env-file` or ensure the file exists at `infrastructure/docker/prod/.env`.
